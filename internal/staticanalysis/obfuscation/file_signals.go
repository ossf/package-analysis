...
// FileSignals hold information related to the presence of obfuscated code in a single file
...
func (s FileSignals) String() string {
	parts := []string{
		...
		fmt.Sprintf("possible base 64 strings: %v", s.Base64Strings),
		fmt.Sprintf("hex strings: %v", s.HexStrings),
		fmt.Sprintf("highly escaped strings: %v", s.EscapedStrings),
	}
...
// ComputeFileSignals creates a Signals object based on the parsing.Data for a given file.
// These signals may be useful to determine whether the code is obfuscated.
func ComputeFileSignals(parseData parsing.Data) FileSignals {
	signals := FileSignals{}
