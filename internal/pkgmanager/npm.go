...
/*
getNPMArchiveFilename generates a filename for a package archive to be
downloaded from NPM. It is generated by trimming any leading '@' and
replacing any '/' characters in the package name with '-' (ref [1]).
The cleaned package name is then concatenated with "-", the version
string and ".tgz".
...
func getNPMArchiveFilename(pkgName, version, _ string) string {
	...
	return fmt.Sprintf("%s-%s.tgz", pkgName, version)
}
